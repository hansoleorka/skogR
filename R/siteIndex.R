...
siteindex <- function(age,HO,SP,method="TVEITE-BRAASTAD"){
  
  if(method=="TVEITE-BRAASTAD"){
    
    # Calculate H40 SPRUCE (Tveite 1977)
    #-------------------------------------------------------------------------------------------------------
    ...
    SIspruce <- ((HO/(diff/3)) - H_G17) + 17 #inverter 
    
    # Calculate H40 PINE (Braastad 1980)
    #-----------------------------------------------------------------------------------------------
    ...
    SIpine <- ((HO/(diff/3)) - H_F14) + 14 #inverter
  ...
  if(method=="SHARMA-BRUNNER"){
    ...
    b2=5175.18;
    b3=1.1576;
    h0 = HO - 1.3# Old version: h0 = si - 1.3;
    a1 = age;
    ...
    #Wrap up
    SIcalc <- rep(NA,length(age))
    SIcalc[SP==1] <- H_sp_new[SP==1]
    SIcalc[SP==2] <- H_pi_new[SP==2]
    SIcalc[SP==3] <- NA
  }
  return(SIcalc)
